...
/**
* This program implements a Highest Priority First (P) scheduling algorithm.
...
void insert(process** head, process newJob) {
    ...
    if (*head == NULL) {
        *head = current;
        ...
        if(prev->priorityLevel < current->priorityLevel){
          current->next = prev;
          *head = current;
          return;
        }
        
        while (prev->next != NULL && prev->next->priorityLevel > current->priorityLevel) {
            prev = prev->next;
        }
...
void print() {
	...
	file = fopen("output_hpf.txt", "a");
	
	if(!file)
	{
		printf("\nError, could not write to file output_hpf.txt");
		exit(1);
	}
...
int main(int argc, char** argv) {
    ...
    while (isCPUFree() == 1 || isQueueEmpty(head) == 0 || processor->jobServiceTime != 0) {
        ...
        else if(head != NULL && processor->jobPriority < head->priorityLevel && processor->jobServiceTime > 0 && isCPUFree() == 0){
