...
/**
* This program implements Shortest Job First (NP) scheduling algorithm.
...
void insert(process** head, process newJob) {
    ...
    if (*head == NULL) {
        *head = current;
        ...
        if(prev->serviceTime > current->serviceTime){
          current->next = prev;
          *head = current;
          return;
        }
        
        while (prev->next != NULL && prev->next->serviceTime < current->serviceTime) {
            prev = prev->next;
        }
...
void print() {
	...
	file = fopen("output_sjf.txt", "a");
	
	if(!file)
	{
		printf("\nError, could not write to file output_sjf.txt");
		exit(1);
	}
